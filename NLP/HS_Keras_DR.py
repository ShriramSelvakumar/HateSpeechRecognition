...
def save_tfidf_scaler(tfidf, scaler):
    ...
    pickle.dump(tfidf.vocabulary_, open(model_path + 'TFIDF/' + 'TFIDF-Vocabulary-Keras_DR' +
                                        datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S") + '.pkl', "wb"))
    print('Saved TFIDF-Keras_DR to Pickle')
    os.makedirs(model_path + 'StandardScaler/', exist_ok=True)
    pickle.dump(scaler, open(model_path + 'StandardScaler/' + 'StandardScaler-Keras_DR' +
                             datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S") + '.pkl', "wb"))
    print('Saved Scaler-Keras_DR to Pickle')
    return
...
y_test, yy = test(data_Test, data_Test['final_label_int'])

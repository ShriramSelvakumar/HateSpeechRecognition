...
def train(X, y, X_valid, y_valid):
    ...
    checkpoint_cb = keras.callbacks.ModelCheckpoint(model_path + "Keras_functional_1.h5", save_best_only=True)
    early_stopping_cb = keras.callbacks.EarlyStopping(patience=10, restore_best_weights=True)

    # Keras Functional model
    input_ = keras.layers.Input(shape=X_train_features.shape[1:])
    ...
    output1 = keras.layers.Dense(2, activation="sigmoid")(hidden4)
    output2 = keras.layers.Dense(4, activation="softmax")(hidden4)
    model = keras.Model(inputs=[input_], outputs=[output1, output2])
    model.compile(loss="sparse_categorical_crossentropy", optimizer="adam", metrics=["accuracy"])
    history = model.fit(X_train_features, (y['binary_label'], y['final_label_int']), epochs=30,
                        validation_data=(X_valid_features, (y_valid['binary_label'], y_valid['final_label_int'])),
...
def test(X, y_binary, y_categorical):
    ...
    try:
        ...
        trained_NN_model = keras.models.load_model(model_path + 'Keras_functional_1' + '.h5')
        ...
        (y_pred_binary, y_pred) = trained_NN_model.predict(X_test_features)
        y_pred_binary = ((np.argmax((y_pred_binary > 0.5).astype("int32"), axis=1)).reshape(-1, 1)).flatten()
        y_pred = np.argmax(y_pred, axis=-1)
        # y_pred = trained_NN_model.predict_classes(X_test_features)
        print("==================================================================================================")
        print('Binary Label -----')
        print("Precision : ", precision_score(y_binary, y_pred_binary))
        print("Recall : ", recall_score(y_binary, y_pred_binary))
...
seq_model.save(model_path + 'Keras_functional_1.h5', save_format='h5')
